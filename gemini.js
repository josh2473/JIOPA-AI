...
/* ═══════════════════════════════════════════════════
   ...
   ─────────────────────────────────────────────────
   Notes:
   - Calls this app's own server (/api/chat, /api/search) instead of
     OpenRouter/Serper directly. API keys now live only on the server.
   - Gemini integration removed (unchanged from before).
═══════════════════════════════════════════════════ */

/* ── ASK OPENROUTER (via server proxy) ── */
async function askOpenRouter(userMessage) {
  setAIStatus('thinking');
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error('Chat proxy error:', response.status, errorData);
      setAIStatus('error');
      setTimeout(() => setAIStatus('live'), 2500);
      return null;
    }
...
/* ── SERPER SEARCH FALLBACK (via server proxy) ── */
function shouldUseSearch(question) {
  const lower = cleanQuestionText(question);
  if (!lower || lower.length < 3) return false;
...
async function askSerper(question) {
  if (!shouldUseSearch(question)) return null;
...
>>>>>>> 2c5d0a5 (Add deployment files and config updates)
